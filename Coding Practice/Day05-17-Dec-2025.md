...
spec:     # Desired configuration of The services.
  containers:
    - name: app
      image: nginx
      resources:
...
pipeline {
    agent any
    environment {     # Block to define environment Variable
        INSTITUTE_NAME = "Pathnex"    # Environment Variable name.
    }
    stages {
        stage('Print') {   # Stage name shown in jenkins UI
            steps {
        ...
        stage('Build') {
            steps {
                sh 'mvn clean package -Dinstitute.name=$INSTITUTE_NAME'  # printing environment name with the help of linuc command.
            }
        }
    }
}
